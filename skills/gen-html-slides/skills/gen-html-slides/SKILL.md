...
**Question 5 — Image Manager** (header: "Images"):
Do you want to add or rearrange images directly in the browser after generation? Options:

- "Yes — Add images later" — Embeds an image manager: upload photos, pick target slide, auto-layout, click to select, drag to reposition, resize
- "No" — Presentation only

**Remember the user's image manager choice — it determines whether image manager code is included in Phase 3.**
...
3. **Summarize** — Tell the user:
   ...
   - If image manager was enabled: Click the 🖼 button (bottom-right) or press **I** to open the panel.
     - **添加图片 tab**: Upload an image, pick a target slide, choose **自由浮动** (absolute overlay — drag to reposition, pull SE corner to resize, × to delete) or **融入布局** (merge into a structured layout slot), then click **添加图片**.
     - **幻灯片布局 tab**: Pick a target slide, select a layout preset (文\|图, 图\|文, 文/图, 图/文, 满屏图, or 纯文字), optionally upload a slot image, then click **应用布局**. After applying, hover the divider line between text and image to reveal it — drag to adjust the split ratio.
     - Use **💾 保存 HTML** to download the updated file with all images embedded.
